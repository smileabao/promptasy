...
## v1.2 · P25a — 打磨：果汁複核 ＋ 音訊 ＋ 無障礙

三件事，彼此獨立。**這一格不加新玩法**，只把既有的東西修齊。

### ① game-feel 複核（只保留不造成壓力的）

v1.2 的鐵則是「**威脅不懲罰、進度只累積**」。果汁（juice）要跟著這條走：
squash／粒子／音層／`pulse` 這種「肯定你做到了」的留著；任何**製造急迫感**的
（畫面震得像被打、紅色警示、逼你快一點的節奏）要找出來並拿掉或減弱。
逐項列出「保留／減弱／拿掉」與理由，不要整批照抄。

### ② 音訊：先修文件矛盾，再看要不要補錄

**文件矛盾（已經釘死正確的一邊，見上面「P25a 前置」）**：
`CLAUDE.md:285` 寫「SFX 峰值 -6 dBFS」是**舊的**；實作與 `WORLD.md` §6.5 一致
（配樂床 −20 LUFS、音效 −19 LUFS、峰值上限 −3 dBFS）。**要改的是 `CLAUDE.md:285`**，
不是實作也不是 WORLD.md。改完確認 `SFX_PEAK_CEILING` 的實際值與文件相符。

**補錄**：`roadmap` 原文寫「12 區 SFX／BGM 補錄」。但實測**13 支配樂音檔全部在位、
23 個音效 cue 有檔且檔案全在（37 個音檔零孤兒）**，另外 31 個 cue 是**刻意用合成的**
（腳步／風鈴／螢火這種每分鐘響很多次的，合成有零下載成本與逐次微變化兩個真好處）。
所以**沒有「缺檔」要補**。這一格要做的是**核對響度**（逐檔重量一次 LUFS／peak，
確認資料層記的數字與檔案相符），不是去補不存在的缺口。**量到不符才動，不要為了動而動。**

### ③ 無障礙

- `prefers-reduced-motion` **全覆蓋**：逐項確認每一個會動的東西都有讓步
  （P23 新加的披肩光點與升等閃光也要）。作法照既有慣例：**保留在傳達資訊的變化，只拿掉位移與掃光**。
- 螢幕閱讀器路徑：面板可達、焦點順序合理、`aria` 標記正確。
- **不要為了無障礙改掉玩法**；找不到乾淨作法的就誠實記下來留給下一輪。

**Acceptance tests（先紅後綠）**
- rubric：`CLAUDE.md` 與 `WORLD.md` 的響度數字**逐值一致**（文件斷言）；
  `SFX_PEAK_CEILING` 與文件相符；每一支音檔的 `lufs`／`peak` 與**重量一次的結果**相符；
  reduced-motion 下該停的停、該留的留（逐項）。
- e2e：reduced-motion 開啟時走一段路 ＋ 過一關，零 console error；焦點順序可走完。

**禁區**：`src/data/*.json`（除非量到響度資料與檔案不符，那時只准改 `audio.js` 裡的量測值並附重量證據）、
`curriculum.json`、`vite.config.js`、`CHANGELOG.md`、`gameplay-roadmap.md`、三件組、dev server 5173／5174／5175。
**`CLAUDE.md` 這一格例外可以改** —— 但只准改 285 行那句響度，其他一個字都不准動。

Exit criteria：
- [ ] 文件矛盾消失且有斷言守著；果汁逐項複核完；reduced-motion 全覆蓋。
- [ ] rubric／playtest／build／e2e 全綠、console error 0；預算逐值不變。

### P25a 收尾（實測）

**① 果汁複核：結論是「沒有東西需要拿掉或減弱」** —— 11 項逐項看過。
關鍵發現：`engine.pulse()` 的 22 個呼叫點**沒有一個掛在失敗或威脅上**
（`onReject`「石碑不收」刻意沒有 pulse）；濁靈眼光的抖動頻率與 aware **無關**
（idle／aware 同頻率）→ 沒有「心跳加快」那條線；全 `src/**` 掃 14 個急迫詞 250 筆命中，
**全部是註解或世界觀文字**，玩家看得到的字串裡一個都沒有。
唯一的判斷題是「石碑不收」那 300ms 的抖＋裂光：留下來的理由是**它是材質回饋不是懲罰**
（不扣分、不失敗、不前進、就地長出金線教學、重選就好），而且 reduced-motion 之下整個關掉。
**站長若認為要減弱，改一行就好。**

**② 響度：逐檔重量 37 支，只有一支不符** —— `bgm_wards.m4a` 的 true peak
資料層記 −3.7、實測 **−3.2**（LUFS −13.2 正確，所以 gain 不動）。
**orchestrator 獨立用 ffmpeg 複量過，確認 −13.2／−3.2。**
會錯的原因：`docs/design/audio-loudness.md` 的配樂表只有 12 列 ——
護欄崗那一首是 issue #3 之後才交付的第 13 支，**從來沒進表**，於是那個數字沒有第二個地方可以對。
補上該列並新增 rubric 逐檔逐值對表（13 配樂 ＋ 26 音效）。
順帶修掉 `WORLD.md` 的六處音訊漂移（§6.5 寫「12 首／34.6 MB」實際 13 支／37.4 MB；
三處區域敘述寫「這一區沒有配樂音檔」但檔案早就在）。**沒有補任何音檔**（本來就沒有缺口）。

**③ reduced-motion：從「大面積沒讓步」修到全覆蓋** ——
修之前只覆蓋 CSS ＋ 8 個世界模組，**整個氛圍動作層與旅人本人完全沒有讓步**。
現在石座浮片／光柱／光環、刻文與殘頁、器物與祕密、起始祭壇、閘門、小景浮階、
地標齒輪、空中塵、貼地霧、極光漂、**旅人的呼吸與重心移動（含 P23 披肩那一圈）** 全部停住；
而**亮度／顏色／不透明度照舊**（那是資訊不是動）—— 包含 P23 披肩的等級格數與升等亮起，
有明確斷言守著「不吃 `calm`」。用 `0` 而不是 `0.12` 的理由：氛圍層是**永遠不停的迴圈**，
打一折仍然永遠在動。

**④ 螢幕閱讀器：抓到並修掉一個真 bug** ——
圖鑑面板 76 顆焦點候選裡 **48 顆 focus 不到**（24 顆是 `<details>` 本身，
24 顆是收起來的 `<details>` 裡的連結：Chrome 走 `::details-content` 的
`content-visibility: hidden`，那是**內部 pseudo-element 不是 DOM 節點**，
所以 `offsetParent`／`getClientRects()`／逐層 `getComputedStyle()` **全部騙得過去**）。
清單最後一顆正是其中之一 → **焦點鎖的 wrap 是空包彈**。修完 76 → 28 顆、focus 不到 **0** 顆。
過程值得記：代理第一次是**用猜的**（以為是 content-visibility），e2e 再紅一次才寫獨立探針逐顆量出真相。

**數字**：rubric **227,274**（+326）／playtest 2,824／build ✓／e2e **4,930 全綠零 console error**／
`audit:perf` **前後 diff 完全空**／fonts 1535.5 KB（CJK 一個字沒變）。
13 種定向破壞逐一驗證新斷言會紅；e2e 新斷言**真的紅過三輪**且每一條紅都是真問題
（其中一條正是 brief 警告的固定 sleep 地雷）。
...
**① e2e flaky 清零 —— 三輪全綠、零重跑**

| 輪 | 結果 |
|---|---|
| baseline（未改動，對照組） | 4,930 通過／0 失敗／0 console error |
| 一／二／三 | **每一輪都 4,930 通過／0 失敗／0 console error** |

根因：整支 `headless-check.mjs` 有 **942 處固定等待**在等牆鐘，而這台機器一幀 174 ms（閒）～481 ms（忙）。
做法分兩層：
- **會咬人的那幾族改成真輪詢**（序章整堂課、風鈴、光菇、拖曳、面板開啟、畫質切換、視窗改尺寸）——
  輪詢一律「先把舊值讀下來再等它翻面」，擋空泛通過。
  風鈴那條特別漂亮：原本從「呼叫 teleport 那一刻」起算 700 ms，**觸發延遲全算在裡面**；
  改成先等 `swing` 從 0 翻正、再從那一刻起算，量到的就變成「衰減 0.7 秒後還剩多少」，與機器速度無關。
  拖曳那三條是 12 個 `pointermove` 擠在同一個 tick，機器一慢就被當成一幀內的抖動 → 改成一幀一步（真滑鼠本來就這樣）。
- **其餘改成 `settle(ms)`／`__paSettle(ms)`**：牆鐘與「N 個真的畫出來的影格」取較久者 ——
  快機器上與原本的 sleep 完全相同，慢機器上才多等，**數學上只會變寬不會變鬆**。
- 刻意留著的 136 處**逐處寫了理由**（多半是輪詢迴圈自己的取樣間隔）。

**沒有刪除或放寬任何一條斷言。** 代價：每輪多約 2 分鐘（21.5 → 23 分）。

**② 低階機 30fps：這台機器證明不了，誠實說不能**

確定性的代理（可重跑）：低/高的一幀 draw call **0.781**（契約上限 0.8）、加色 0.777、三角 0.599。
幀時也量了並寫明條件（SwiftShader、1280×800、丟掉前 30 幀、取 80 幀中位數）：
整頁以該畫質載入 → 高 218.6／232.0 ms、低 174.6～185.5 ms，比值 ≈ 0.78，**與 draw call 比值互相印證**。

**但發現一件要記住的事**：在遊戲中**即時切畫質**（不重載）時，高 208 ms、低 **378 ms** ——
**低畫質反而慢 1.8 倍**，兩次交錯量測都重現。原因幾乎確定是 `WebGLRenderer({ antialias: true })`：
高畫質渲進 EffectComposer 的無 MSAA target，低畫質直接畫到**開了 MSAA 的預設 framebuffer**，
而 SwiftShader 的 MSAA resolve 走 CPU。**這是軟體光柵器的產物，真 GPU 上結論可能相反**
（低階 GPU 上 4×MSAA 便宜、bloom 貴），所以**沒有改任何東西** —— 留給下一個動畫質開關的人。
...
| 濁靈 | 8 | **20（其中 12 隻大濁靈）** |
| 高台 | 5 | **11** |
| 自架字型 | 1.44／1.47 MB | **1,535.5 KiB** |
| rubric 斷言 | 「8 萬+」 | **227,274**（41.9 s） |
| playtest 耗時 | 「~10 秒」 | **0.13 s**（差兩個數量級） |
| e2e 項數／耗時 | 「3,300+」／15–20 分 | **4,930**／23 分 |

**orchestrator 獨立複核過石碑 14、高台 11、濁靈 20（大濁靈 12）、地標 12 —— 全部相符。**
`expected-counts.json` **一個字沒動**（所有實測值都與契約相符；有出入才是迴歸，不該用改契約蓋掉）。

**④ 兩處「越界」的裁決（orchestrator）：保留**
- 動了 `AGENTS.md`：它是 CLAUDE.md harness 的鏡像版，留著矛盾的響度數字就是**製造新的不一致**
  ——正是這一輪修掉兩次的那類 bug。
- 改寫了「已知 flaky → 重跑一次即可」那條策略：e2e 現在三輪零重跑，**留著它會直接抵銷這一格的成果**。
  新的寫法是「動畫時序類斷言不准等牆鐘」，並說明 `settle()` 的用法與「留著固定 sleep 要寫理由」。

**⑤ 誠實留著的**
- **797 處固定等待沒有全部改成條件輪詢** —— 只改了會咬人的那幾族，其餘用 `settle`。
  逐站點推導條件要對近 800 個位置各做一次產品判斷，風險與成本遠高於收益；
  而 `settle` 只會等更久，不可能讓任何斷言變得比較容易過。**這一點不含糊帶過。**
- README 第 56 行「Captured from the current **v1.1** build」沒動 —— 截圖確實還是 v1.1 拍的，改了就是說謊。
