...
| 指令 | 內容 | 耗時 | 什麼時候跑 |
|---|---|---|---|
| `npm run test:rubric` | 22 萬+ 斷言：評分引擎、資料完整性、出處健檢、碰撞審計、中文掃描、字型語料指紋、存檔遷移 | ~42 秒 | **每次改動都跑**（最便宜的安全網） |
| `npm run test:playtest` | 每關「照提示一定過得了」的門檻：範例解 ≥A、快速填入必過、弱起手必不過、誤判迴歸案例 | < 1 秒 | 動到關卡資料 / 檢查器時 |
| `npm run build` | Vite 建置 | ~2 秒 | 每次改動都跑 |
| `npm run test:e2e` | 4,900+ 項無頭瀏覽器實玩（走路、序章、刻碑、過關、分享…） | **20–30 分鐘**（此機為 SwiftShader 軟體渲染，機器閒著時實測 174 ms/幀，忙起來 372–481 ms/幀） | 大改動、動到互動流程時 |

### 測試策略（重要 — 這是成本控制的核心）

- **先問再跑**：如果這次改動**項目不多**（純文案、單一元件樣式、資料微調…），先詢問使用者要「全跑 / 只跑快的（rubric＋build）/ 不跑」，不要預設把 20–30 分鐘的 e2e 跑下去。使用者明說「不用跑測試」就照辦（但 rubric＋build 這種十幾秒的快檢通常仍值得跑，除非連這個也被排除）。
- **不重複驗證**：實作交給 subagent 時，subagent 跑過全綠的 e2e，orchestrator **不要再跑一次完整 e2e**——只做快速驗證（rubric ＋ build ＋ curl dev server 確認 200）。
- **動畫時序類斷言不准等牆鐘**（v1.2 · P25b 把整支 e2e 的固定 sleep 掃過一遍）：等的是**條件成立**（poll until，要有超時、而且不能被前一個值滿足），不是「等 N 毫秒」。真的只是「讓畫面跑一拍」的，用 `settle(ms)`／`window.__paSettle(ms)`——牆鐘與「N 個真的畫出來的影格」一起等，快機器上和原本的 sleep 相同、慢機器上才多等。**留著固定 sleep 的地方要在旁邊寫清楚為什麼**（多半是輪詢迴圈自己的取樣間隔）。以前那批「拖曳／火盆亮度／風鈴擺動」偶發紅燈就是等牆鐘等出來的，已在 P25b 逐條根治——**不要再用「重跑一次」當解法**。
